...
```{r horse-pop-plot, echo = FALSE, message = FALSE, warning = FALSE, fig.align="center", out.width="95%", fig.cap= "Horse populations per provinces from 1906 - 1972"}
...
Additionally, note that in Table \@ref(tab:sd-table) we consider the sample standard deviation of the number of horses during the same time span as Figure \@ref(fig:horse-pop-plot).

```{r large-sd, echo = FALSE, fig.align="center", out.width="60%", fig.cap= "Standard deviations of horse populations for all provinces in Canada from 1906 - 1972"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In Figure \@ref(fig:large-sd) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
